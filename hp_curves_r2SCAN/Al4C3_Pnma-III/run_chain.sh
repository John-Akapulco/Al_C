#!/bin/sh
# r2SCAN H(P) scan for Al4C3_Pnma-III: 11 pressure points, each already
...
#SBATCH --job-name=r2scan_Pnma-III
#SBATCH --nodes=1
#SBATCH --ntasks=48
#SBATCH --cpus-per-task=1
#SBATCH --output=r2scan_Pnma-III.log
...
run_step() {
    ...
    if ! grep -q "General timing" OUTCAR 2>/dev/null; then
        echo "$(date '+%F %T') VASP did not finish cleanly in $dir -- aborting chain" >> "$BASE/r2scan_Pnma-III.log"
        exit 1
    fi
...
run_step "P90"
run_step "P100"
echo "$(date '+%F %T') === Al4C3_Pnma-III: r2SCAN 0-100 GPa series done ==="

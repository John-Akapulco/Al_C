#!/bin/sh
#SBATCH --job-name=hp_R-3m
#SBATCH --nodes=1
#SBATCH --ntasks=48
#SBATCH --cpus-per-task=1
#SBATCH --output=hp_R-3m.log
...
run_step() {
    ...
    if ! grep -q "General timing" OUTCAR 2>/dev/null; then
        echo "$(date '+%F %T') VASP did not finish cleanly in $dir -- aborting chain" >> "$BASE/hp_R-3m.log"
        exit 1
    fi
...
run_step "P90" "P80"
run_step "P100" "P90"
echo "$(date '+%F %T') === Al4C3_R-3m: full 0-100 GPa series done ==="

#!/bin/sh
# 800eV/0.05 combined PBE+r2SCAN H(P) chain for Al4C3_Fd-3m: 0-70 GPa
...
#SBATCH --job-name=hires_Fd-3m
#SBATCH --nodes=1
#SBATCH --ntasks=48
#SBATCH --cpus-per-task=1
#SBATCH --output=hires_Fd-3m.log
...
run_step() {
    ...
    if ! grep -q "General timing" OUTCAR 2>/dev/null; then
        echo "$(date '+%F %T') VASP did not finish cleanly in $dir -- aborting chain" >> "$BASE/hires_Fd-3m.log"
        exit 1
    fi
...
echo "$(date '+%F %T') === Al4C3_Fd-3m: 800eV/0.05 PBE+r2SCAN 0-70 GPa chain done ==="
